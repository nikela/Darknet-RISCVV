       ...
       int w_offset = c % ksize;
        ...
        for (h = 0; h < height_col; ++h) {
            ...
            for (w = 0; w < width_col; ) {
                ...
                __epi_2xi32 OFFSET = IBCAST(w_offset, gvl); //broadcast
                __epi_2xi32 PAD = IBCAST(pad, gvl); //broadcast
                __epi_2xi32 STRIDE = IBCAST(stride, gvl); //broadcast
                ...
                __epi_2xi32 WIDTHCOL = IBCAST(width_col, gvl); //broadcast
                __epi_2xi32 INTER = IBCAST(intermediate, gvl); //broadcast
                ...
                __epi_2xi32 WIDTH = IBCAST(width, gvl); //broadcast
                __epi_2xi32 HEIGHT = IBCAST(height, gvl); //broadcast
                __epi_2xi32 CIM = IBCAST(c_im, gvl); //broadcast
                __epi_2xi32 imrow = IBCAST(im_row, gvl); //broadcast


                //Broadcast 4 for index calculation (index*4 for float 32bit)
                 int l = 4;
                 __epi_2xi32 FOUR = IBCAST(l, gvl); //broadcast
                ...
                //__builtin_epi_vstore_2xi32(&col_index[w+0], colindex, gvl);
                 ...
                 int z=0;
                float z1=0.0;
                __epi_2xi32 XERO = IBCAST(z, gvl);  //broadcast
                __epi_2xf32 XERO1 = BCAST(z1, gvl);  //broadcast
                ...
                __epi_2xi32 intermediate5 = IBCAST(val, gvl); //broadcast
